...
Version **0.1.15**, early development. Login and live video work end to end
...
- Talk has been built from the official app's captured messages but is the
  least tested part.
- Presets saved by an older Reoling (before 0.1.13) have no picture until
  one is taken in their Edit dialog.

**Version history**

- 0.1.15 — presets are listed oldest first, newest last: the camera does not
  say when each was added, so the order each id was first seen in is
  remembered (`~/.config/reoling/preset-order-*.txt`), with new presets
  appended and given ids after the highest in use. A preset the official app
  put on id 0 (its first free one) is older by that rule and stays on top.
...
- 0.1.13 — Add and Edit now do what the official app does, from a decrypted
  ...
  it; clicking the picture in Edit takes a new one the same way; Confirm
  renames (a preset message with no `command`) and attaches the new picture.
  Uploads go out with the JPEG in the clear, like Talk.
